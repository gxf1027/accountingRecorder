...
@Repository("rpcAuthenInfoDao")
public class RpcAuthenInfoDaoImpl implements RpcAuthenInfoDao{
	...
	public Map<String, String> getAuthenInfo(String username) {
		String sql = "SELECT username, confidential_code "
				+ "FROM  rpc_authentication WHERE username = :username AND yxbz='Y'";
		...
		return namedTemplate.query(sql, paramMap, new ResultSetExtractor<Map<String, String>>() {
			...
			public Map<String, String> extractData(ResultSet rs) throws SQLException, DataAccessException {
				...
				if (map.size() > 1){
					throw new SQLException("cn.gxf.spring.motan.dao.RpcAuthenInfoDaoImpl.getAuthenInfo 返回非唯一结果");
				}
				return map;
			}
		});
	...
	public Map<String, String> getAuthenInfo() {
		String sql = "SELECT username, confidential_code "
				+ "FROM  rpc_authentication WHERE yxbz='Y'";
		Map<String, Object> paramMap = new HashMap<>();
	...
	public int accessValidating(String username, String interfaceName, String methodName) {
		...
		String sql = "SELECT COUNT(1) FROM rpc_authentication a, rpc_interface b, rpc_interface_control c "
				+ " WHERE a.username = c.rpc_username AND c.rpc_interface_uuid = b.uuid "
				+ " AND a.username = :username AND b.rpc_interface_name = :interfaceName AND b.rp_method_name = :methodName "
